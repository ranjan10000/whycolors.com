...
const COLOR_PICKER_ID = 'palette-color-picker'; // ✅ Unique ID
...
export default function PaletteClient({
  ...
  useEffect(() => {
    ...
    const h1Name = document.getElementById('palette-h1-name');
    const h1Hex = document.getElementById('palette-h1-hex');

    if (h1Name) h1Name.textContent = currentColorName;
    if (h1Hex) h1Hex.textContent = currentColor;

    const breadcrumbDot = document.getElementById('palette-breadcrumb-dot');
    const breadcrumbHex = document.getElementById('palette-breadcrumb-hex');

    if (breadcrumbDot) {
      breadcrumbDot.style.backgroundColor = currentColor;
    }
    if (breadcrumbHex) {
      breadcrumbHex.textContent = currentColor;
    }

    document.title = `${currentColorName} Color Palettes (${currentColor})`;
    ...
    (newColor: string) => {
      ...
      try {
        ...
        window.history.pushState(
          {},
          '',
          `/color/palettes/${cleanHex.toLowerCase()}`
        );
        ...
        <div className="space-y-6">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="animate-pulse">
              ...
              ></div>
              <div
                className={`grid grid-cols-5 gap-0 rounded-xl overflow-hidden border ${
  ...
  return (
    ...
    >
      ...
      {/* Header Section — H1 REMOVED (server-rendered in page.tsx) */}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 w-full">
          ...
          <div className="space-y-3 text-center sm:text-left w-full">
            ...
            {/*
              ...
              Only the badges stay (dynamic, not SEO-critical)
            */}
      ...
      <div className="grid grid-cols-1 gap-6">
        {paletteTypes.map((type) => {
          const isBaseColor = type.colors.some(
            ...
            >
              ...
              {viewMode === 'strip' && (
                <div className="space-y-3">
                  <button
                    onClick={() =>
                      handleCopyAll(type.colors, type.label)
                    }
