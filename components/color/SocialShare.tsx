...
interface SocialShareProps {
  hex: string;
  colorName: string;
  isDark?: boolean; // ✅ FIX 1: Made optional
}

export default function SocialShare({
  hex,
  colorName,
  isDark: isDarkProp, // ✅ FIX 2: Rename prop to avoid name clash
}: SocialShareProps) {
  // ✅ FIX 3: Fallback to ThemeContext if prop not passed
  const { isDark: isDarkContext } = useTheme();
  const isDark = isDarkProp ?? isDarkContext;

  const [copied, setCopied] = useState(false);
  const [showShare, setShowShare] = useState(false);
  const [shareUrl, setShareUrl] = useState(''); // ✅ FIX 4: SSR-safe URL state
  const dropdownRef = useRef<HTMLDivElement>(null);

  const fullHex = `#${hex.toUpperCase()}`;
  const shareText = `🎨 Check out this beautiful color ${fullHex} (${colorName})!`;

  // ✅ FIX 5: Set shareUrl after mount (avoids SSR mismatch)
  useEffect(() => {
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            <button
              ...
              role="menuitem"
            >
              {/* ✅ FIX 6: use isDark state instead of Tailwind dark: */}
              <span className={isDark ? 'text-white' : 'text-[#000000]'}>
                <SocialIcons.Twitter />
              </span>
